...
def fetch_realtime_df(account: str, password: str, inverter_sn: str, req_date: date, columns: list[str],
    ...
    """Executa crosslogin e busca várias colunas via GetInverterDataByColumn."""
    ...
    for col in columns:
        try:
            ...
            if not df_col.empty:
                ...
                st.warning(
                    f"Não consegui parsear a coluna '{col}'. Veja JSON bruto abaixo.")
...
# Streamlit gera classes específicas, então miramos nelas.
st.markdown("""
<style>
    /* Cor principal para botões e detalhes */
    :root {
...
if df is not None and not df.empty:
    ...
    if res.get("hora_pico"):
        col2.metric("Pico de potência", kw(res.get("pico_potencia", 0.0)),
    ...
    with left:
        if "Pac" in df.columns:
            fig_p = px.line(df, x="time", y="Pac", markers=True,
                            title="Potência (Pac) ao longo do dia")
...
with st.form("add_device_form"):
    ...
    if submitted:
        ...
        if name and consumption > 0:
            ...
            st.success(f"✅ Dispositivo '{name}' adicionado!")
        else:
...
for dev_id, dev in list(st.session_state.devices.items()):
    ...
    col1.write(f"**{dev['name']}** — {dev['consumption_kw']} kW")
    col2.write("🟢 Ligado" if dev["state"] == "on" else "⚪ Desligado")
    ...
    if col4.button("❌ Remover", key=f"del_{dev_id}"):
...
if "devices" not in st.session_state:
    st.session_state.devices = {
        "base01": {"id": "base01", "name": "Consumo Base", "state": "on", "consumption_kw": 0.3},
        "lamp01": {"id": "lamp01", "name": "Lâmpada Sala", "state": "off", "consumption_kw": 0.1},
...
def gerar_ar_html(facing_mode: str, consumo_total: float) -> str:
    ...
    async function initCamera() {{
        try {{
            ...
            console.log("Erro ao acessar câmera:", e);
        }}
    }}
    initCamera();
    </script>
...
if st.session_state.is_mobile:
    facing = st.radio("Escolha câmera", ["Traseira", "Frontal"])
    facing_mode = "environment" if facing == "Traseira" else "user"
    html(gerar_ar_html(facing_mode, calcular_consumo_total()), height=400)
else:
    st.warning(
        "⚠ AR só pode ser usado em dispositivos Mobile. A câmera não será ativada no desktop.")
